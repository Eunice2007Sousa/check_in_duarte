...
function AtletaDashboard({
  ...
  // É esta que alimenta tanto o cartão de perfil
  // (lá em cima) como o aviso junto às turmas do dia.
  ...
  // Percentagem para a barra do TOPO — segue a semana
  // do dia selecionado no calendário, não a de hoje.
  const pctSemanaSelecionada = resumoSemanaSelecionada
    ? Math.max(
        0,
        Math.min(
          100,
          (resumoSemanaSelecionada.usados /
            resumoSemanaSelecionada.frequencia) *
            100
        )
      )
    : 0;
    ...
    <main className="px-6 py-8 max-w-3xl w-full mx-auto space-y-6">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 flex items-center justify-between flex-wrap gap-3">
        ...
        <div>
          ...
          {/* INFORMAÇÃO FIXA DO PLANO — não muda com o dia
              selecionado no calendário, é sempre a frequência
              semanal atual do atleta (mês corrente). */}
          {resumoSemanaAtual && (
            <div className="text-xs text-zinc-500 mt-0.5">
              Plano atual:{" "}
              <span className="text-lime-400 font-medium">
                {resumoSemanaAtual.frequencia} treino
                {resumoSemanaAtual.frequencia > 1 ? "s" : ""}/semana
              </span>
            </div>
          )}

          {/* BARRA — segue a semana do dia selecionado no
              calendário (não fica presa à semana de hoje). */}
          <div className="text-sm text-zinc-500 mt-1">
            {resumoSemanaSelecionada ? (
              <>
                Semana {formatDiaMes(resumoSemanaSelecionada.semana_inicio)} –{" "}
                {formatDiaMes(resumoSemanaSelecionada.semana_fim)}:{" "}
                {resumoSemanaSelecionada.usados} de{" "}
                {resumoSemanaSelecionada.frequencia} treino
                {resumoSemanaSelecionada.frequencia > 1 ? "s" : ""} usados
                {" — "}
